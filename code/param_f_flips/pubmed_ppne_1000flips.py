...
parser.add_argument('--save',default='./pubmed_ppne_flips_1000', type=str)
...
with open(os.path.join(args.save, 'ppne_1000flips_modify'), 'a') as file_record:
    file_record.write('flips 1000, now time: {}\n'.format(datetime.now()))
    file_record.write('-------------------------------------------\n')
    file_record.flush()

with open(os.path.join(args.save, 'ppne_1000flips_final'), 'a') as file_record_final:
    file_record_final.write('pubmed, deepwalk, 1000flips, now time: {}\n'.format(datetime.now()))
...
for _ in range(1):
    ...
    for i in range(ITER):
        ...
        adds_or_deletes = np.random.choice([0, 1], 1000, p=[1 - args.add_delete_prob, args.add_delete_prob])
        add_num = np.sum(adds_or_deletes)
        del_num = 1000 - add_num
