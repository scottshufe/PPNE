...
char network_file[MAX_STRING], embedding_file[MAX_STRING], context_embedding_file[MAX_STRING];
...
void Output()
{
	...
	FILE *fo = fopen(context_embedding_file, "wb");
	fprintf(fo, "%d %d\n", num_vertices, dim);
	for (int a = 0; a < num_vertices; a++)
	{
		fprintf(fo, "%s ", vertex[a].name);
...
int main(int argc, char **argv) {
	...
	if ((i = ArgPos((char *)"-output2", argc, argv)) > 0) strcpy(context_embedding_file, argv[i + 1]);
