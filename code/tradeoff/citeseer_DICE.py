...
parser.add_argument('--save',default='./tradeoff_citeseer_DICE', type=str)
...
#------------------------------------------------------------------------------------------------
# Make the split.
adj_train = np.load('../citeseer_data/adj_train.npy')
test_edges = np.load('../citeseer_data/test_edges.npy')
test_edges_false = np.load('../citeseer_data/test_edges_false.npy')
labels = np.load('../citeseer_data/labels.npy')
...
target_edges = list(graph_first.edges(target_nodes))


with open(os.path.join(args.save, 'citeseer_dice_modify'), 'a') as file_record:
    file_record.write('dice modify, now time: {}\n'.format(datetime.now()))
    file_record.write('-------------------------------------------\n')
    file_record.flush()
...
for _ in range(1):
    adds_or_deletes = np.random.choice([0, 1], 1000, p=[0.5, 0.5])
    ...
    for i in range(len(adds_or_deletes)):
        ...
        if adds_or_deletes[i] == 0:
            # random del an edge between target nodes
            while True:
                del_idx = np.random.choice(len(target_edges), 1)[0]
                del_edge = target_edges[del_idx]
                x, y = del_edge
                ...
                if len(list(nx.isolates(train_graph_copy))) == 0:
                    print("ITER {} random delete edge {}".format(i, del_edge))
                    adj_train_first = adj_train_copy
                    num_deleted_edges += 1
                    target_edges.remove(del_edge)
                    break
                else:
                    print("delete make a node isolation. repick.")
                    continue
