...
parser.add_argument('--candidate-add-num', type=int, default=100000)
parser.add_argument('--save',default='./pubmed_ppne_batch_size_100k', type=str)
...
with open(os.path.join(args.save, 'ppne_batch_size_100k_modify'), 'a') as file_record:
    file_record.write('batch size 100k, now time: {}\n'.format(datetime.now()))
    file_record.write('-------------------------------------------\n')
    file_record.flush()

with open(os.path.join(args.save, 'ppne_batch_size_100k_final'), 'a') as file_record_final:
    file_record_final.write('pubmed, deepwalk, 100flips, now time: {}\n'.format(datetime.now()))
...
for _ in range(1):
    ...
    for i in range(ITER):
        ...
        adds_or_deletes = np.random.choice([0, 1], 100, p=[1 - args.add_delete_prob, args.add_delete_prob])
