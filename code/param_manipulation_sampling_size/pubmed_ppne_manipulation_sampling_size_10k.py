...
parser.add_argument('--save',default='./pubmed_ppne_batch_size_10k', type=str)
...
with open(os.path.join(args.save, 'ppne_batch_size_10k_modify'), 'a') as file_record:
    file_record.write('batch size 10k, now time: {}\n'.format(datetime.now()))
    file_record.write('-------------------------------------------\n')
    file_record.flush()

with open(os.path.join(args.save, 'ppne_batch_size_10k_final'), 'a') as file_record_final:
