...
def deepwalk(args, g, verbose=True, random_seed=0):
  ...
  if data_size < args.max_memory_data_size:
    ...
    if verbose:
      print("Training...")
    model = Word2Vec(walks, size=args.dim, window=args.window_size, seed=random_seed, min_count=0, sg=1, hs=0, iter=args.iter, workers=args.workers)
  else:
    if verbose:
...
def deepwalk_v2(args, g, verbose=True, random_seed=0):
  ...
  if data_size < args.max_memory_data_size:
    ...
    if verbose:
      print("Training...")
    model = Word2Vec(walks, size=args.dim, window=args.window_size, seed=random_seed, min_count=0, sg=1, hs=1, iter=args.iter, workers=args.workers)
  else:
    if verbose:
      print("Data size {} is larger than limit (max-memory-data-size: {}).  Dumping walks to disk.".format(data_size, args.max_memory_data_size))
      print("Walking...")
