...
with open(os.path.join(args.save, 'cora_privacy_modify'), 'a') as file_record:
...
for _ in range(1):
    ...
    for i in range(ITER):
        ...
        if adds_or_deletes[i] == 1:
            ...
            # privacy vals
            priv_vals = abs(y_ts_grad[pos_grad_indice])
            max_idx = pos_grad_indice[np.argmax(priv_vals)]
            ...
            priv_vals = abs(y_ts_grad[neg_grad_indice])
            max_idx = neg_grad_indice[np.argmax(priv_vals)]
